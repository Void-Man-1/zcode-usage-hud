...
- Preview runs longer than one refresh tick need the previewMode guard at the
top of refreshNow — without it a real fetch clobbers the preview snapshot at
the 5s tick and the bar restacks to 340x40 mid-harness.
- Harness "initial rect" must be a SETTLED rect (two consecutive equal samples
at 150ms): the launch collapse animation passes through 372x210-style frames
that false-FAIL "expected a bar" assertions.
- Physical mouse clicks on the HUD are unreliable in harnesses (snap/restack
timer moves the window; multi-monitor DPI skews SetCursorPos). Post
WM_LBUTTONDOWN/UP directly with painter-space client coords instead — that is
the exact hit-test path. settings_check.ps1 has the pattern.
- FindWindowW from PowerShell Add-Type returns 0 for the HUD window even when
EnumWindows sees it (works for Shell_TrayWnd). Always locate windows by
pid+class enum (Get-HudWindow in settings_check.ps1); set CharSet=Unicode on
string P/Invokes regardless.
- Collapsed-strip buttons (gear at [W-108,W-72]) only exist in collapsed
geometry — wait for width<500 after launch or the click hits the expanded
title strip and expands the bar instead.
- `& $exe | Out-Null` on a GUI exe BLOCKS PowerShell until exit — it hung the
install E2E for 5 minutes. Launch with Start-Process; wait via Get-Process.
...
- Code is split: main.go (window/paint/API), animate.go (animation engine),
theme.go+prefs.go (theme + settings.json), standby.go+standby_paint.go
(outage alarm), gauge.go (speedometer style), settings.go (settings window),
update.go (GitHub updater), alpha.go (AlphaBlend fades), common.go. A
behavior change usually lands in exactly one of these.
- ISS version keys must derive from {#MyAppVersion} — literal
VersionInfoVersion=1.5.0.0 once survived a version bump and shipped lying
file properties.
- scripts/dev/install_e2e.ps1: full silent install→upgrade-over-running→
uninstall-while-running cycle (~10s, hash-asserts the exe, no prompts).
