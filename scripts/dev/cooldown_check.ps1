...
try {
  ...
  while ($sw.Elapsed.TotalSeconds -lt $watchSecs) {
    ...
    if ($initialW -eq 0) { $initialW = $w; $initialH = $ht }

    if ($mode -ne 'expanded') {
      # alarm blinks ON the minimized bar itself
      if (-not $sawRed -and $t -ge 0.7 -and $t -le 6.2 -and $w -le 400) {
      ...
      if ($initialW -gt 0 -and ($w -ne $initialW -or $ht -ne $initialH)) { $anyResizeFromExpanded = $true }
